...
// TODO: DELETE after migration is confirmed — temporary one-time migration function, not for production.
// Migrates phoneSettings and phonePlaybook from malformed nested paths to canonical account-level paths.
export const migratePhonePaths = functions
  .runWith({ invoker: "public" })
  .https.onRequest(async (req, res) => {
  if (req.query.key !== "0490c5f9bdfa52898c6eca1c2436b4da") {
    res.status(403).json({ error: "Forbidden" });
    return;
  }

  const isDryRun = req.query.dry !== "false";

  const ACCOUNT_ID = "88shkNYerSR4SjfaI86G";
  const EXPECTED_SETTINGS_PATH = "accounts/88shkNYerSR4SjfaI86G/members/YIkqIryP6raAAFLWlzC7ePriled2/phoneSettings/YIkqIryP6raAAFLWlzC7ePriled2";
  const EXPECTED_PLAYBOOK_PATH = "accounts/88shkNYerSR4SjfaI86G/members/YIkqIryP6raAAFLWlzC7ePriled2/phoneSettings/YIkqIryP6raAAFLWlzC7ePriled2/phonePlaybook/YIkqIryP6raAAFLWlzC7ePriled2";
  const TARGET_SETTINGS_PATH = `accounts/${ACCOUNT_ID}/phoneSettings/${ACCOUNT_ID}`;
  const TARGET_PLAYBOOK_PATH = `accounts/${ACCOUNT_ID}/phonePlaybook/${ACCOUNT_ID}`;

  const report: Record<string, any> = { dryRun: isDryRun, steps: [] };

  // Fetch all non-canonical docs via collectionGroup
  const [allSettings, allPlaybook] = await Promise.all([
    db.collectionGroup("phoneSettings").get(),
    db.collectionGroup("phonePlaybook").get(),
  ]);

  const nonCanonicalSettings = allSettings.docs.filter((d) => d.ref.path !== TARGET_SETTINGS_PATH);
  const nonCanonicalPlaybook = allPlaybook.docs.filter((d) => d.ref.path !== TARGET_PLAYBOOK_PATH);

  if (nonCanonicalSettings.length !== 1) {
    report.steps.push({ status: "ABORT", reason: `Expected exactly 1 non-canonical phoneSettings doc, found ${nonCanonicalSettings.length}: ${nonCanonicalSettings.map((d) => d.ref.path).join(", ")}` });
    res.status(200).json(report);
    return;
  }
  if (nonCanonicalPlaybook.length !== 1) {
    report.steps.push({ status: "ABORT", reason: `Expected exactly 1 non-canonical phonePlaybook doc, found ${nonCanonicalPlaybook.length}: ${nonCanonicalPlaybook.map((d) => d.ref.path).join(", ")}` });
    res.status(200).json(report);
    return;
  }

  const settingsDoc = nonCanonicalSettings[0];
  const playbookDoc = nonCanonicalPlaybook[0];

  // Assert exact source paths
  if (settingsDoc.ref.path !== EXPECTED_SETTINGS_PATH) {
    report.steps.push({ status: "ABORT", reason: `phoneSettings path mismatch: expected "${EXPECTED_SETTINGS_PATH}", got "${settingsDoc.ref.path}"` });
    res.status(200).json(report);
    return;
  }
  if (playbookDoc.ref.path !== EXPECTED_PLAYBOOK_PATH) {
    report.steps.push({ status: "ABORT", reason: `phonePlaybook path mismatch: expected "${EXPECTED_PLAYBOOK_PATH}", got "${playbookDoc.ref.path}"` });
    res.status(200).json(report);
    return;
  }

  // Assert accountId via path split — never parent.parent.id (returns uid on malformed paths)
  const settingsAccountId = settingsDoc.ref.path.split("/")[1];
  const playbookAccountId = playbookDoc.ref.path.split("/")[1];
  if (settingsAccountId !== ACCOUNT_ID) {
    report.steps.push({ status: "ABORT", reason: `phoneSettings accountId mismatch: expected "${ACCOUNT_ID}", got "${settingsAccountId}"` });
    res.status(200).json(report);
    return;
  }
  if (playbookAccountId !== ACCOUNT_ID) {
    report.steps.push({ status: "ABORT", reason: `phonePlaybook accountId mismatch: expected "${ACCOUNT_ID}", got "${playbookAccountId}"` });
    res.status(200).json(report);
    return;
  }
  ...
  if (settingsData.twilioPhoneNumber !== "+18446878016") {
    report.steps.push({ status: "ABORT", reason: `phoneSettings.twilioPhoneNumber expected "+18446878016", got "${settingsData.twilioPhoneNumber}"` });
    res.status(200).json(report);
    return;
  }
  if (playbookData.businessName !== "Harris Plumbing") {
    report.steps.push({ status: "ABORT", reason: `phonePlaybook.businessName expected "Harris Plumbing", got "${playbookData.businessName}"` });
    res.status(200).json(report);
    return;
  }

  report.steps.push({
    step: "source_validated",
    settingsPath: settingsDoc.ref.path,
    settingsData,
    playbookPath: playbookDoc.ref.path,
    playbookData,
  });
  ...
  if (isDryRun) {
    report.steps.push({
      ...
      wouldWrite: [TARGET_SETTINGS_PATH, TARGET_PLAYBOOK_PATH],
      wouldDelete: [EXPECTED_PLAYBOOK_PATH, EXPECTED_SETTINGS_PATH],
    });
    res.status(200).json(report);
    return;
  }
  ...
  // Abort if targets already exist (idempotency guard)
  const [existingSettings, existingPlaybook] = await Promise.all([
    db.doc(TARGET_SETTINGS_PATH).get(),
    db.doc(TARGET_PLAYBOOK_PATH).get(),
  ]);
  if (existingSettings.exists) {
    report.steps.push({ status: "ABORT", reason: `Target ${TARGET_SETTINGS_PATH} already exists — aborting to avoid overwrite` });
    res.status(200).json(report);
    return;
  }
  if (existingPlaybook.exists) {
    report.steps.push({ status: "ABORT", reason: `Target ${TARGET_PLAYBOOK_PATH} already exists — aborting to avoid overwrite` });
    res.status(200).json(report);
    return;
  }

  // Write to canonical paths
  await db.doc(TARGET_SETTINGS_PATH).set(settingsData);
  await db.doc(TARGET_PLAYBOOK_PATH).set(playbookData);

  // Read back and verify field-by-field
  const [verifySettings, verifyPlaybook] = await Promise.all([
    db.doc(TARGET_SETTINGS_PATH).get(),
    db.doc(TARGET_PLAYBOOK_PATH).get(),
  ]);

  if (!verifySettings.exists || !verifyPlaybook.exists) {
    report.steps.push({ status: "ABORT", reason: "Read-back after write failed — docs missing. NOT deleting source." });
    res.status(200).json(report);
    return;
  }

  const verifiedSettingsData = verifySettings.data()!;
  const verifiedPlaybookData = verifyPlaybook.data()!;

  const settingsMismatches = Object.keys(settingsData).filter(
    (k) => JSON.stringify(settingsData[k]) !== JSON.stringify(verifiedSettingsData[k]),
  );
  const playbookMismatches = Object.keys(playbookData).filter(
    (k) => JSON.stringify(playbookData[k]) !== JSON.stringify(verifiedPlaybookData[k]),
  );

  if (settingsMismatches.length > 0) {
    report.steps.push({ status: "ABORT", reason: `phoneSettings field mismatch after write on keys: ${settingsMismatches.join(", ")} — NOT deleting source` });
    res.status(200).json(report);
    return;
  }
  if (playbookMismatches.length > 0) {
    report.steps.push({ status: "ABORT", reason: `phonePlaybook field mismatch after write on keys: ${playbookMismatches.join(", ")} — NOT deleting source` });
    res.status(200).json(report);
    return;
  }

  report.steps.push({ step: "writes_verified", settings: TARGET_SETTINGS_PATH, playbook: TARGET_PLAYBOOK_PATH });

  // Delete sources — playbook first (nested deeper)
  await playbookDoc.ref.delete();
  await settingsDoc.ref.delete();

  report.steps.push({
    step: "complete",
    deleted: [EXPECTED_PLAYBOOK_PATH, EXPECTED_SETTINGS_PATH],
    created: [TARGET_SETTINGS_PATH, TARGET_PLAYBOOK_PATH],
  });
