...
**Version:** 1.8 · **Last updated:** July 27, 2026 · **Status:** Living document. Referenced by Claude and Claude Code every session. Updated deliberately, not per-sprint.

**v1.8 change (July 27, 2026):** §8 **S4** flipped from "not yet done" to **done across the spend paths in production** — the inbound voice path now gates deterministically at the `/twiml` boundary via a per-caller + global-daily cost cap (`services/conversation-relay/phone-rate-limit.js`), with per-call duration/turn bounds in `/ws` and deny-all Rules on the root-level `phoneRateLimits`/`phoneUsage` counters. First status change to a §8 SOP since v1.0. No practice in §§0–7, 9–11 changed; §§6–7 remain PROPOSED.

**v1.7 change (July 26, 2026):** Added a §10 environment fact from the Session W hosting-gap fix — `firebase.json` `hosting` is now an array of two target objects (`marketing` → `h3-website/public`, `dashboard` → `web/dist` with an SPA rewrite), mapped via `.firebaserc`; the dashboard deploy is two steps, target-scoped (`npm run build` then `firebase deploy --only hosting:dashboard`), never a bare `--only hosting`; and a new Hosting site's `.web.app` domain must be added to Auth authorized domains by hand. No practice in §§0–9, 11 changed.

**v1.6 change (July 25, 2026):** Added two §10 environment facts from the ConversationRelay production cutover (Session V) — (1) Cloud Run's Google Front End **reserves `/healthz`** and returns a GFE 404 before the request reaches the container, so external health checks and uptime monitors must use a different liveness path (`/livez`); (2) latency-sensitive Cloud Run services (voice especially) should run **`--min-instances 1`** to avoid a cold-start pause on the first call. No practice in §§0–9, 11 changed; §§6–7 remain PROPOSED.
...
**S4 — Rate limiting \+ cost caps on anything that spends money.** Per-caller and global limits; `maxInstances` on every function; GCP billing alert and Twilio usage trigger. **Status: done across the spend paths in production today** — the signup/checklist HTTP functions (rate-limited when the site shipped) and the **inbound voice path** (shipped July 27, 2026, commit `e098f9c`). The voice path gates deterministically at the **`/twiml` boundary** — after Twilio signature validation, before account lookup / the paid `/ws` \+ Gemini path — via `services/conversation-relay/phone-rate-limit.js`: per-caller hourly/daily limits plus a load-bearing **global daily ceiling**, with per-call duration/turn bounds enforced in the `/ws` loop. Counters are **root-level** (`phoneRateLimits`, `phoneUsage`) — a documented exception to §2.1 tenant-scoping, since a rate limit gates *before* the caller's account is known or trusted; Admin-SDK writes only, deny-all Rules as defense-in-depth. On a limiter (Firestore) error the gate **fails open** — a database blip must not take down the live receptionist line; worst-case spend stays bounded by the billing budget, Twilio usage trigger, and `--max-instances 2`. (The broader pre-self-serve hardening — bot detection, email verification, trial-abuse limits — is tracked separately from S4 and **remains open** before self-serve signup.)
...
- **Two hosting sites, two targets — dashboard vs marketing.** `firebase.json` `hosting` is an **array of two target objects**, not a single object. `marketing` → `h3-website/public` (site `h3operations-prod`); `dashboard` → `web/dist` (site `h3operations-app`), with an SPA rewrite `**` → `/index.html`. Targets are mapped in `.firebaserc`. Dashboard deploy is **two steps, target-scoped**: `cd web && npm run build` then (from repo root) `firebase deploy --only hosting:dashboard`. **Never** bare `firebase deploy --only hosting` — it deploys both sites. **Never** deploy without building first — deploying stale/unbuilt `web/dist` at an untargeted scope is what shipped the wrong site pre-Session W (the subscriber dashboard had never actually reached prod despite being reported "shipped"). A new Firebase Hosting site's `.web.app` domain is **not** auto-added to Auth authorized domains — add it under **Auth → Settings → Authorized domains** or login silently bounces with no error.  
    
- **Cloud Run reserves `/healthz` at the Google Front End — it returns a GFE 404 *before* the request reaches your container.** Use a different liveness path (`/livez`) for external health checks and uptime monitors. On the `conversation-relay` service every path reached the app (app `text/plain` 404, and it appeared in the container request log) **except** `/healthz`, which returned Google's own `text/html` 404 (the ~1568-byte robot page) with **no** container request-log entry — proof the GFE swallows it upstream. The health handler was coded correctly; it simply never runs for `/healthz`. Diagnose this class of issue by **content-type**: the app never emits HTML, so an HTML 404 = GFE interception, not your code (`/health`, `/livez`, `/readyz`, and arbitrary paths all reach the app; only `/healthz` is intercepted). Renaming the route to `/livez` (returns `200 ok`) made it reachable and gave the cutover uptime check a valid target. (Production cutover, 2026-07-25.)  
    
- **Latency-sensitive Cloud Run services (voice especially) should run `--min-instances 1`.** A scaled-to-zero service pays a cold start on the first request after idle; on the live receptionist line this was heard as an **~11s pause on the first call**, with the immediately-following call clean — proving cold start, not code, was the delay. `gcloud run services update <svc> --region <r> --min-instances 1` keeps one warm instance (a small always-on cost — the founder's call) so a caller never eats the cold start. Confirm it applied via `minScale: 1` in `spec.template.metadata.annotations['autoscaling.knative.dev/minScale']`. (Production cutover, 2026-07-25.)  
    
- **Phone-path rate limiting (§8 S4) gates at the `/twiml` boundary, before the costly `/ws` socket + Gemini turn loop ever opens.** `services/conversation-relay/phone-rate-limit.js` checks/consumes three limits in order: per-caller hourly (5) then per-caller daily (10) — keyed on `From` E.164 digits, collapsing anonymous/withheld/unparseable caller IDs to a shared `'anonymous'` bucket — then, only if the caller passes, a global daily ceiling (150) that bounds worst-case spend regardless of how many caller IDs an attacker rotates through. **DECISION:** counters live at root-level (`phoneRateLimits`, `phoneUsage`), NOT tenant-scoped — a deliberate, documented exception to §2.1's "everything scopes by account," because the gate runs before we know or trust the caller's account. Firestore Rules deny all client read/write on both collections (defense-in-depth; Admin SDK bypasses Rules for the real writes). **DECISION:** fails OPEN on a limiter error (a Firestore blip must not take down the live receptionist line; worst-case spend still bounded by the GCP billing budget, Twilio usage trigger, and `--max-instances 2`). Per-call bounds (Layer 3 — 5 min max duration, 25 max turns) live in the `/ws` loop itself, independent of the `/twiml`-boundary caps. **Known residual (accepted, not solved here):** `--max-instances 2` means a concurrency-saturation attack can still block real callers before spend caps trip — the global cap bounds spend, not concurrency starvation. (Phone-path rate limiting, 2026-07-27.)
...
*v1.8 — July 27, 2026\. §8 S4 status flipped to done for the spend paths in production — signup/checklist (already rate-limited) and the inbound voice path. The voice path gates at the /twiml boundary (per-caller hourly/daily + global daily ceiling) before the paid /ws + Gemini path opens, with per-call duration/turn bounds in /ws; counters are root-level phoneRateLimits/phoneUsage (documented §2.1 exception), Admin-SDK-write-only, deny-all client Rules, covered by service unit tests + firestore-tests Rules tests. Fails open on limiter error (availability over guard; outer backstops bound spend). Broader pre-self-serve bot-abuse defense tracked separately and still open. No practice in §§0–7, 9–11 changed.*

*v1.7 — July 26, 2026\. Added a §10 environment fact from the Session W hosting-gap fix: `firebase.json` `hosting` is an array of two target objects (`marketing` → `h3-website/public`, site `h3operations-prod`; `dashboard` → `web/dist`, site `h3operations-app`, with an SPA rewrite `**` → `/index.html`), mapped via `.firebaserc`. Dashboard deploy is two steps, target-scoped (`cd web && npm run build` then `firebase deploy --only hosting:dashboard`); never a bare `firebase deploy --only hosting`, and never deploy without building first — that gap is what shipped the wrong site pre-Session W despite the subscriber dashboard being reported "shipped". A new Hosting site's `.web.app` domain must be added to Auth authorized domains by hand or login silently bounces. No practice in §§0–9, 11 changed.*

*v1.6 — July 25, 2026\. Added two §10 environment facts from the ConversationRelay production cutover (Session V) — (1) Cloud Run's Google Front End reserves `/healthz` and returns a GFE 404 before the container, so external health checks and uptime monitors must use a different liveness path (`/livez`); diagnose by content-type (an HTML 404 = GFE interception, not app code). (2) Latency-sensitive Cloud Run services should run `--min-instances 1` to avoid a cold-start pause on the first request (heard as an ~11s pause on the first live call, clean thereafter). No practice in §§0–9, 11 changed; §§6–7 remain PROPOSED.*
